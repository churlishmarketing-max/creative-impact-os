...
export default function PayPage() {
  ...
  const wrap: React.CSSProperties = {
    minHeight: "100vh", background: "#0a1322", color: "#f4f7fc",
    display: "flex", alignItems: "center", justifyContent: "center", padding: "24px",
    fontFamily: "'Archivo', sans-serif",
  };
  const card: React.CSSProperties = { width: "520px", maxWidth: "100%", background: "#101d33", border: "1px solid #24385c", borderTop: "3px solid #ffb81c" };

  if (loading) return <div style={wrap}><div style={{ color: "#5c7096", fontSize: 13 }}>Loading invoice…</div></div>;
  if (err && !inv) return <div style={wrap}><div style={{ color: "#8ea3c4", fontSize: 13, textAlign: "center" }}>{err}</div></div>;
  if (!inv) return <div style={wrap}><div style={{ color: "#8ea3c4" }}>Not found.</div></div>;
      ...
      <div style={card}>
        <div style={{ padding: "22px 26px", borderBottom: "1px solid #24385c", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 8.5, letterSpacing: ".28em", color: "#5c7096", marginTop: 3 }}>INVOICE {inv.number}</div>
          </div>
        </div>

        <div style={{ padding: "24px 26px" }}>
          {inv.client ? <div style={{ fontSize: 11, letterSpacing: ".14em", color: "#5c7096", textTransform: "uppercase", marginBottom: 4 }}>Billed to {inv.client}</div> : null}
          {inv.title ? <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 800, fontSize: 26 }}>{inv.title}</div> : null}

          <div style={{ margin: "18px 0", border: "1px solid #33455f" }}>
            {(inv.items || []).map((it, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 13px", borderBottom: i < inv.items.length - 1 ? "1px solid #16263f" : "none", fontSize: 13 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 13px", background: "#060c17" }}>
              <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 900, fontSize: 18, letterSpacing: ".02em" }}>TOTAL</div>
              <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 900, fontSize: 22, color: "#ffb81c" }}>{money(inv.amount_cents)}</div>
            </div>
          </div>
          ...
          {paid ? (
            <div style={{ background: "#0c1f14", border: "1px solid #1d3d2a", color: "#2ee06f", textAlign: "center", padding: "14px", fontSize: 13, letterSpacing: ".1em", textTransform: "uppercase" }}>✓ Paid — thank you</div>
          ) : (
            <button onClick={pay} disabled={busy} style={{ width: "100%", background: "#ffb81c", color: "#1a1608", border: "none", padding: 14, fontFamily: "'Archivo', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: ".16em", textTransform: "uppercase", cursor: busy ? "default" : "pointer", opacity: busy ? 0.6 : 1 }}>
              {busy ? "Starting checkout…" : "Pay " + money(inv.amount_cents) + " →"}
            </button>
          )}
          {err ? <div style={{ color: "#ffb81c", fontSize: 12, marginTop: 12, textAlign: "center", lineHeight: 1.4 }}>{err}</div> : null}
          <div style={{ fontSize: 10, color: "#5c7096", marginTop: 16, textAlign: "center" }}>Secured by Stripe · Creative Impact</div>
        </div>
      </div>
    </div>
  );
