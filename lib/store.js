...
class SupabaseStore {
  ...
  async loadAll() {
    const sb = getBrowserClient();
    await this._uidOnce();
    const [sprintR, dealsR, weeksR, appR, logR, clientsR, invoicesR, proposalsR, bookingsR, expensesR, kpisR, kpiEntriesR, shootsR, delivsR] = await Promise.all([
      ...
      sb.from("shoots").select("*").order("shoot_date", { ascending: true }),
      sb.from("deliverables").select("*").order("created_at", { ascending: true }),
    ]);
    ...
    // Broadcast additions (tables from 16_broadcast.sql; empty if not migrated yet).
    const shoots = (shootsR.data || []).map((r) => ({
      id: r.id,
      date: r.shoot_date,
      client: r.client || "",
      kind: r.kind || "",
    ...
    const delivs = (delivsR.data || []).map((r) => ({
      id: r.id,
      name: r.name,
      done: r.clips || 0,
      meta: r.meta || "",
    ...
    return { sprint, deals, weeks, app, log, clients, invoices, proposals, bookings, expenses, kpis, kpiEntries, shoots, delivs };
  }

  async upsertShoot(sh) {
    const sb = getBrowserClient();
    ...
    if (sh.id) {
      const { error } = await sb.from("shoots").update(row).eq("id", sh.id);
      if (error) throw new Error("updateShoot: " + error.message);
      return sh.id;
    }
    const { data, error } = await sb.from("shoots").insert(row).select("id").single();
    if (error) throw new Error("insertShoot: " + error.message);
    return data?.id || null;
  }

  async deleteShoot(id) {
    const sb = getBrowserClient();
    const { error } = await sb.from("shoots").delete().eq("id", id);
    if (error) throw new Error("deleteShoot: " + error.message);
  }

  async upsertDeliv(d) {
    const sb = getBrowserClient();
    const uid = await this._uidOnce();
    const row = { user_id: uid, name: d.name || "", clips: +d.done || 0, meta: d.meta || "" };
    if (d.id) {
      const { error } = await sb.from("deliverables").update(row).eq("id", d.id);
      if (error) throw new Error("updateDeliv: " + error.message);
      return d.id;
    }
    const { data, error } = await sb.from("deliverables").insert(row).select("id").single();
    if (error) throw new Error("insertDeliv: " + error.message);
    return data?.id || null;
  }

  async deleteDeliv(id) {
    const sb = getBrowserClient();
    const { error } = await sb.from("deliverables").delete().eq("id", id);
    if (error) throw new Error("deleteDeliv: " + error.message);
  }
...
class LocalStore {
  ...
  async deleteKpi() {}
  async setKpiEntry() {}
  async upsertShoot(sh) { return sh.id || "local-" + Math.random().toString(36).slice(2, 9); }
  async deleteShoot() {}
  async upsertDeliv(d) { return d.id || "local-" + Math.random().toString(36).slice(2, 9); }
  async deleteDeliv() {}
